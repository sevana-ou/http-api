...
const std::string Html_Ok = "<html><body><p>Status: {status}</p><p>Message: {message}</p></body></html>";
...
void dashboard_start(int port, std::atomic_bool& exit_flag)
{
    ...
    DashboardServer->set_handler([&exit_flag](http_server& server, void* ctx, const http_server::request_info& info, const http_server::request_params& params)
    {
